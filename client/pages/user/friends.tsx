...
const Friends: React.FunctionComponent<FriendsProps> = () => {
        ...
        const handleOnRequest = React.useCallback(async (username: string, status: RelationshipStatus) => {
                                                ...
                                                <div key={user.id} className="flex items-center justify-between ">
                                                        ...
                                                        <div className="flex space-x-2">
                                                                <button
                                                                        ...
                                                                        onClick={() => handleOnRequest(user.username, RelationshipStatus.FRIEND)}
                                                                ></button>
                                                                <button
                                                                        className="block w-10 h-10 bg-yellow-500 rounded-full"
                                                                        onClick={() => handleOnRequest(user.username, RelationshipStatus.NONE)}
                                                                ></button>
                                                                <button
                                                                        className="block w-10 h-10 bg-red-500 rounded-full"
                                                                        onClick={() => handleOnRequest(user.username, RelationshipStatus.BLOCK)}
                                                                ></button>
                                                        </div>
                                                </div>
                                        ))}
